...
function generateInvoicePDF($invoiceId, $lawFirmId) {
    ...
    // Set gray background color for headers
    $pdf->SetFillColor(200, 200, 200); // Light gray color

    // Create headers with gray background and centered text
    $pdf->Cell(90, 6, 'Description', 1, 0, 'C', true);
    $pdf->Cell(25, 6, 'Quantity', 1, 0, 'C', true);
    $pdf->Cell(35, 6, 'Unit Price', 1, 0, 'C', true);
    $pdf->Cell(40, 6, 'Total', 1, 1, 'C', true);

    $pdf->SetFont('Arial', '', 10);
    // Reset fill color for content rows
    ...
    foreach ($invoice_items as $item) {
        ...
        $pdf->Cell(35, 6, formatCurrency($item['price']), 1, 0, 'R');
        $pdf->Cell(40, 6, formatCurrency($item['total']), 1, 1, 'R');
    }
    ...
    // Add a short line after tax rate, starting from the right
    $lineY = $pdf->GetY();
    $pdf->Line(170, $lineY, 200, $lineY); // Adjust the starting X coordinate (170) to change the line length
    $pdf->Ln(1); // Add a small space after the line
    ...
    if (!empty($invoice['pdfFilePath']) && file_exists($invoice['pdfFilePath'])) {
        unlink($invoice['pdfFilePath']);
    }

    // Update the invoice record with the new PDF file path
    $update = $connect->prepare("UPDATE invoices SET pdfFilePath = ? WHERE id = ?");
