<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $client_tpin = filter_input(INPUT_POST, 'selected_clientIdTpin', FILTER_SANITIZE_SPECIAL_CHARS);
        $lawFirmId = filter_input(INPUT_POST, 'lawFirmId', FILTER_SANITIZE_SPECIAL_CHARS);
        $caseCategory = filter_input(INPUT_POST, 'caseCategory', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        $caseCategory = encrypt($caseCategory);
        ...
        try {
            ...
            if (!empty($caseId)) {
                // Update case
                $sql = $connect->prepare("UPDATE cases SET caseNo=?, causeId = ?, caseTitle=?, caseCategory=?, caseStatus=?, caseDescription=?, caseDate=?, feeMethod=?, fixedFee=?, currency=?, hourly_rate=?, created_at=?, other_case_status = ? WHERE id=?");
                $sql->execute([$caseNo, $causeId, $caseTitle, $caseCategory, $caseStatus, $caseDescription, $caseDate, $feeMethod, $fixedFee, $currency, $hourlyRate, $createdAt, $customStatus, $caseId]);
                ...
                // Insert new case
                $sql = $connect->prepare("INSERT INTO cases (userId, lawFirmId, clientId, client_tpin, caseNo, causeId, caseTitle, caseCategory, caseStatus, caseDescription, caseDate, feeMethod, fixedFee, currency, hourly_rate, other_case_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $sql->execute([$userId, $lawFirmId, $clientId, $client_tpin, $caseNo, $causeId, $caseTitle, $caseCategory, $caseStatus, $caseDescription, $caseDate, $feeMethod, $fixedFee, $currency, $hourlyRate, $customStatus]);
