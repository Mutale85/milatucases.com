										            ...
										            <?php
										            	...
										            	if($count > 0){
										                	foreach ($query->fetchAll() as $row){
											                	...
											                	$invoiceId = $row['id']; 
											                	$filePath = fetchInvoicePDF($invoiceId, $lawFirmId, $clientId);
											                	$link = preg_replace("#[^0-9.A-Za-z-]#", " ", $filePath);
											                	$link = removeFirstWord($link);
											                	if($row['status'] == '0'){
											                		$status = '<button data-invoice="'.$invoiceId.'" data-number="'.$row['invoice_number'].'" id="'.$row['total'].'" class="btn btn-danger btn-sm updatePayment">Unpaid</button>';
											                	}else{
										                ...
										                ?>
										                    <tr>
																					...
																					<td><?php echo date("D d M, Y", strtotime($row['due_date'])); ?></td>
										                        <td>
