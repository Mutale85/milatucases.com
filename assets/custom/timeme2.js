(() => {
	((root, factory) => {
		...
		let TimeMe = {
			...
			setUpWebsocket: (websocketOptions, userId, lawFirmId) => {
				if (window.WebSocket && websocketOptions) {
					let websocketHost = websocketOptions.websocketHost;
					try {
						TimeMe.websocket = new WebSocket(websocketHost);
						/*
						TimeMe.setUpWebsocket = (websocketOptions, userId, lawFirmId) => {
						    if (window.WebSocket && websocketOptions) {
						        let websocketHost = websocketOptions.websocketHost;
						        try {
						            TimeMe.websocket = new WebSocket(websocketHost);
						            ...
						            TimeMe.websocket.onopen = () => {
						                ...
						                TimeMe.sendInitWsRequest(websocketOptions.appId, userId, lawFirmId);
						            };
						            ...
						            TimeMe.websocket.onclose = (event) => {
						                console.log("WebSocket connection closed", event);
						            };

						            window.onbeforeunload = () => {
						                TimeMe.sendCurrentTime(websocketOptions.appId, userId, lawFirmId);
						            };
			...
			sendCurrentTime: (appId, userId, lawFirmId) => {
			...
			sendInitWsRequest: (appId, userId, lawFirmId) => {
				let data = {
					type: "INIT",
					appId: appId,
					userId: userId,
			...
			initialize: (options) => {
				...
				TimeMe.setUpWebsocket(websocketOptions, options.userId, options.lawFirmId)
...
function getPageNameFromUrl(url, includeFolder = false) {
    // Parse the URL
    ...
    if (includeFolder && pathParts.length > 1) {
