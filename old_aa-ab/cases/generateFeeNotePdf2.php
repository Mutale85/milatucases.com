 <?php
    ...
    function fetchTimerLogs($caseId, $clientId, $lawFirmId) {
        global $connect;
        $sql = "SELECT * FROM `timer_logs` WHERE case_id = ? AND lawFirmId = ?";
        $stmt = $connect->prepare($sql);
    ...
    class FeeNotePDF extends Fpdi {
        ...
        function Header() {
            ...
            $this->Cell(0, 5, 'Cause No: ' . $this->clientInfo['caseNo'], 0, 1);
        ...
        function AddFeeDetails() {
            ...
            $this->Cell(30, 10, 'Amount (ZMW)', 1, 1, 'C');

            $this->SetFont('Arial', '', 9);
            $total = 0;
            foreach ($this->feeNoteDetails['items'] as $item) {
                $this->Cell(30, 8, date('d/m/Y', strtotime($item['created_at'])), 1);
                $this->Cell(70, 8, $item['task_description'], 1);
                $this->Cell(30, 8, $item['time_spent'], 1, 0, 'C');
