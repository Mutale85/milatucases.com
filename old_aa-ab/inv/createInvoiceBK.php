<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($companyInfo as $row) {
            ...
            $companyInfoArray[] = str_replace('<br>', "\n", $row['address']);
        ...
        $pdf->Cell(150, 7, 'Tax (' . $tax_rate . '% ' . $tax_type . '):', 0, 0, 'R'); // Display tax type
        ...
        $pdf->SetY(-60); // Set the Y position for Terms above Notes
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(0, 7, 'Terms:', 0, 1); // Change 'Notes' to 'Terms'
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(0, 5, $terms);

        // Add Notes
        $notes = html_entity_decode($notes);
        $pdf->SetY(-40); // Set the Y position for Notes below Terms
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(0, 7, 'Notes:', 0, 1); // Change 'Notes' to 'Notes'
        $pdf->SetFont('Arial', '', 9);
