document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: 'events/fetchEventsCalendar',
        editable: true,
        eventTimeFormat: {
            hour: '2-digit',
            minute: '2-digit',
    ...
    $(document).on("click", ".removeEvent", function(event) {
        ...
        if (confirm("Are you sure you want to delete this event?")) {
            $.ajax({
                url: 'events/deleteSelectedEvent',
                type: 'POST',
                data: { eventId: eventId },
                success: function(data) {
                    // Handle success response
                    sweetSuccess('Event deleted successfully');
                    calendar.refetchEvents();
                    callAddedEvents();
    ...
    function showEditEventModal(event) {
        ...
        $.ajax({
            url: 'events/fetchSelectedEventDetails',
            type: 'POST',
            ...
            success: function(data) {
                if (data.event) {
                    ...
                    // Handle attendees
                    $("input[name='attendees[]']").prop('checked', false); // Uncheck all attendees
                    data.attendees.forEach(function(attendeeId) {
                        $("#user_" + attendeeId).prop('checked', true);
                    });
           ...
           return false; 
            ...
            if(navigator.onLine){
                $.ajax({
                    ...
                    success: function(response) {
                        if(response.includes("Event deleted successfully")){
                            calendar.refetchEvents()
                            callAddedEvents();
    ...
    */
        ...
        if (result.isConfirmed) {
          if (navigator.onLine) {
            $.ajax({
              ...
              success: function(response) {
                if (response.includes("Event deleted and notifications sent")) {
                  calendar.refetchEvents();
                  callAddedEvents();
                  sweetSuccess("Event Deleted and alerts sent to attendees");
                }
            ...
            if (navigator.onLine) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#eventForm')[0].reset();
                        sweetSuccess("Event saved successfully!");
    ...
    function callAddedEvents(){
        ...
        $.ajax({
            url: 'events/fetchAddedEvent', // Your PHP script to process the event
            type: 'POST',
        ...
        $.ajax({
            url: 'events/fetchWeeklyEvents', // Your PHP script to process the event
            type: 'POST',
            data: {eventData: eventData},
            success: function(response) {
...
function fetchTodaysCalendar() {
   var today = new Date().toJSON().slice(0, 10);
    $.ajax({
        url: 'events/fetchTodaysCalendar',
        type: 'POST',
        data: { today:today },
        success: function(response) {
            $("#todaysEvents").html(response);
        },
        error: function(xhr, status, error) {
